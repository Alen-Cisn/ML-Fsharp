...
type Mode = StaticMode | PlayerMode | RandomMode

type Action = MoveLeft | MoveRight | MoveUp | MoveDown | NoOp

type Piece = Player | Goal | Pit | Wall

type Gridworld(size, mode) as self =
    let board =
    ...
    do
        ...
        match mode with
        | StaticMode -> self.InitGridStatic()
        | PlayerMode -> self.InitGridPlayer()
        | RandomMode -> self.InitGridRandom()
